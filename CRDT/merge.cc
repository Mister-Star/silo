...
void Merge::Init() { /// [shard][epoch]
    ...
    epoch_result_returned_queue_vec.resize(CRDTContext::kWorkerThreadNum);
    ...
    for(int i = 0; i < static_cast<int>(CRDTContext::kWorkerThreadNum); i ++) {
        epoch_result_returned_queue_vec[i] = std::make_shared<
                BlockingConcurrentQueue<std::shared_ptr<CRDTTransaction>>>();
    }
}
...
void Merge::ShardInit(const uint64_t &shard) { /// [shard][epoch]
    ...
    for(int i = 0; i < static_cast<int>(max_length); i ++) {
        (*epoch_merge_map_vec[shard])[i] = std::make_unique<concurrent_crdt_unordered_map<
                std::string, std::string, std::string>>();
        (*epoch_abort_map_vec[shard])[i] = std::make_unique<concurrent_crdt_unordered_map<
                std::string, std::string, std::string>>();

        (*epoch_txn_map_vec[shard])[i] = std::make_unique<concurrent_unordered_map<
                std::string, std::shared_ptr<CRDTTransaction>>>();

        (*epoch_read_validate_queue_vec[shard])[i] = std::make_unique<BlockingConcurrentQueue<
                std::shared_ptr<CRDTTransaction>>>();
        (*epoch_merge_queue_vec[shard])[i] = std::make_unique<BlockingConcurrentQueue<
                std::shared_ptr<CRDTTransaction>>>();
        (*epoch_commit_queue_vec[shard])[i] = std::make_unique<BlockingConcurrentQueue<
                std::shared_ptr<CRDTTransaction>>>();
        (*epoch_redo_log_queue_vec[shard])[i] = std::make_unique<BlockingConcurrentQueue<
                std::shared_ptr<CRDTTransaction>>>();
        (*epoch_result_return_queue_vec[shard])[i] = std::make_unique<BlockingConcurrentQueue<
                std::shared_ptr<CRDTTransaction>>>();
    }
...
void Merge::MergeThreadLocalInit(const uint64_t &shard) {
    ...
    total_exec_txn_num_shard = CRDTCounters::total_exec_txn_num_vec[shard];
    total_exec_latency_shard = CRDTCounters::total_exec_latency_vec[shard];
    total_read_validate_txn_num_shard = CRDTCounters::total_read_validate_txn_num_vec[shard];
    total_read_validate_latency_shard = CRDTCounters::total_read_validate_latency_vec[shard];
    total_read_validate_txn_num_shard = CRDTCounters::total_merge_txn_num_vec[shard];
    ...
    total_result_return_txn_num_shard = CRDTCounters::total_result_return_txn_num_vec[shard];
    total_result_return_latency_shard = CRDTCounters::total_result_return_latency_vec[shard];
...
//                continue; ///only for debug
    ...
    if (!result) {
        total_read_version_check_failed_txn_num_shard->fetch_add(1);
    }
...
//    total_commit_txn_num_shard->fetch_add(1);
//    total_commit_latency_shard->fetch_add(now_to_us() - time1);
//    if(result) {
//        success_commit_txn_num_shard->fetch_add(1);
//        success_commit_latency_shard->fetch_add(now_to_us() - time1);
//    }
...
//                std::cerr << "merge dequeue read_validate" << shard_id << " epoch " << epoch << std::endl;
                ValidateReadSet();
                txn_ptr->crdt_txn->read_validate_time = txn_ptr->crdt_txn->t.lap();
                txn_ptr.reset();
                sleep_flag = false;
            }
        }
...
//                    std::cerr << "merge dequeue sharded_txn CRDTMerge"<< shard_id << " epoch " << epoch << std::endl;
                    CRDTMerge();
                    txn_ptr->crdt_txn->merge_time = txn_ptr->crdt_txn->t.lap();
                    txn_ptr.reset();
                    sleep_flag = false;
                }
            }
...
//                    std::cerr << "merge dequeue sharded_txn Commit"<< std::endl;
                    Commit();
                    txn_ptr->crdt_txn->commit_time = txn_ptr->crdt_txn->t.lap();
                    txn_ptr.reset();
                    sleep_flag = false;
                }
            }
...
//                    std::cerr << "merge dequeue txn ResultReturn"<< std::endl;
                    ...
                    epoch_result_returned_queue_vec[txn_ptr->crdt_worker_id]->enqueue(txn_ptr);
                    epoch_result_returned_txn_num_shard->IncCount(epoch_mod, 1);
                    sleep_flag = false;

                    auto exec_t = total_exec_latency_shard->fetch_add(txn_ptr->exec_time);
                    auto shard_t = total_exec_latency_shard->fetch_add(txn_ptr->shrad_time);
                    auto read_t = total_read_validate_latency_shard->fetch_add(txn_ptr->read_validate_time);
                    auto merge_t = total_merge_latency_shard->fetch_add(txn_ptr->merge_time);
                    auto commit_t = total_commit_latency_shard->fetch_add(txn_ptr->commit_time);
                    auto result_t = total_result_return_latency_shard->fetch_add(txn_ptr->t.lap());
                    auto total_t = total_commit_txn_num_shard->fetch_add(1);

                    if(epoch % CRDTContext::print_mode_size == 0) {
                        std:: string s = "shard " + std::to_string(shard_id) + " total exec txn num " + std::to_string(total_t)
                                                  + " avg exec_time " + std::to_string(exec_t / total_t)
                                                  + " avg shard_time " + std::to_string(shard_t / total_t)
                                                  + " avg read_validate_time " + std::to_string(read_t / total_t)
                                                  + " avg merge_time " + std::to_string(merge_t / total_t)
                                                  + " avg commit_time " + std::to_string(commit_t / total_t)
                                                  + " avg result_time " + std::to_string(result_t / total_t);
                        std::cerr << s << std::endl;
                    }
