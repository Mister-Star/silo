...
bool CRDTCounters::StaticInit() {
    ...
    total_merge_txn_num_vec.resize(shard_num); ///[shard][epoch]
...
bool CRDTCounters::StaticInitShard(uint64_t& shard) {
    ...
    total_merge_txn_num_vec[shard] = std::make_shared<AtomicCounters>(max_length);
    total_merge_latency_vec[shard] = std::make_shared<AtomicCounters>(max_length);
    total_commit_txn_num_vec[shard] = std::make_shared<AtomicCounters>(max_length);
    total_commit_latency_vec[shard] = std::make_shared<AtomicCounters>(max_length);
    success_commit_txn_num_vec[shard] = std::make_shared<AtomicCounters>(max_length);
    success_commit_latency_vec[shard] = std::make_shared<AtomicCounters>(max_length);
...
bool CRDTCounters::StaticClear(uint64_t& epoch) {
    ...
    ClearAllThreadLocalCountNum(epoch, total_merge_txn_num_vec);
    ClearAllThreadLocalCountNum(epoch, total_merge_latency_vec);
    ClearAllThreadLocalCountNum(epoch, total_commit_txn_num_vec);
    ClearAllThreadLocalCountNum(epoch, total_commit_latency_vec);
    ClearAllThreadLocalCountNum(epoch, success_commit_txn_num_vec);
    ClearAllThreadLocalCountNum(epoch, success_commit_latency_vec);
    ClearAllThreadLocalCountNum(epoch, total_read_version_check_failed_txn_num_vec);
    ClearAllThreadLocalCountNum(epoch, total_failed_txn_num_vec);
...
bool CRDTCounters::CheckEpochResultReturned(const uint64_t& epoch) {
    if (epoch_result_returned[epoch % max_length]->load()) return true;
    if (epoch < EpochManager::GetPhysicalEpoch() && IsRecordCommitted(epoch) && IsResultReturned(epoch)) {
        epoch_result_returned[epoch % max_length]->store(true);
        return true;
    }
    return false;
}
